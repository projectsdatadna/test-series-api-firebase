...
CRITICAL SECTION COVERAGE REQUIREMENT:
- Analyze ALL sections provided in the input chunk
- Include ALL main concepts and subconcepts from the section
- Do NOT skip or ignore any section or concept
- Do NOT limit to 4-6 concepts - expand as needed to cover all content
- Each subconcept must be fully visible without truncation
- File size increase is acceptable to ensure complete coverage

CRITICAL CARD OVERFLOW PREVENTION:
- Each subconcept must be fully visible without truncation
- No text should be cut off or truncated
- Allow mind map to expand vertically and horizontally as needed
- Use min-height: auto; overflow: visible; for all elements
- Ensure all concepts are fully visible without cropping
...
REQUIREMENTS:
- Generate mind map concepts for ALL concepts in the section (no fixed limit)
...
- Ensure all SVG icons and ASCII diagrams display completely
- No text should be cut off or truncated
- Allow cards to expand vertically to fit content
- Ensure all cards are fully visible without cropping
...
- Every card title MUST start with a relevant emoji (?? ?? ?? ?? ?? ?? ? ??)
