...
function getStickyNotesPrompt(params) {
  ...
  return `Extract key concepts from the section ${sectionNumber} in the chapter to form the basis of an HTML study guide and Generate a SINGLE A4 PAGE HTML script that visually presents a set of sticky notes for each key concept. Each sticky note must display concise content with clear information and supporting details. The design should be visually appealing and easy to read, with a consistent layout and color scheme. The sticky notes should be organized in a grid layout, with each card having a uniform size and shape. The overall design should be professional and suitable for use in an educational or training setting. notes of the file with ${contentDepth} content depth in ${outputLanguage} language in ${contentType} style with ${visualStyle} nature as a structured, visually elegant, and interactive reference sheet using Tailwind CSS. The layout should serve as a quick-access knowledge companion for students and professionals — focused on clarity, visual memory cues, and ease of scanning. 

CRITICAL EMOJI RENDERING REQUIREMENT:
To ensure emojis render correctly in the generated image, you MUST use Twemoji library:
1. Add this script in the <head> section BEFORE the closing </head> tag:
   <script src="https://unpkg.com/twemoji@latest/dist/twemoji.min.js" crossorigin="anonymous"></script>
2. Add this CSS in the <style> section to control emoji size:
   img.emoji {
     height: 1em;
     width: 1em;
...
3. Add this script at the END of <body> section BEFORE the closing </body> tag:
   <script>
...
function getReadyReckonerPrompt(params) {
  ...
  return `Extract key concepts from the section ${sectionNumber} in the chapter to form the basis of an HTML study guide and Generate an HTML script that visually presents a ready reckoner notes of the file with topicName ${topicName} with ${contentDepth} content depth in ${outputLanguage} language in ${contentType} style with ${visualStyle} nature as a structured, visually elegant, and interactive reference sheet using Tailwind CSS. The layout should serve as a quick-access knowledge companion for students and professionals — focused on clarity, visual memory cues, and ease of scanning. 

CRITICAL A4 PAGE REQUIREMENTS:
1. Page Size: Use CSS @page rule with size: A4 portrait (210mm × 297mm). Set body margin to 0 and use padding on content container.
2. Page Container: Wrap all content in a div with class "page" that has exact dimensions: width: 210mm, min-height: 297mm, padding: 15mm, box-sizing: border-box.
3. Multi-Page Support: If content exceeds one A4 page, create multiple page divs. Use CSS page-break-after: always on each page div except the last one.
4. Print Styles: Include @media print rules to ensure proper page breaks: @media print { .page { page-break-after: always; } .page:last-child { page-break-after: auto; } }
5. Content Density: Optimize content to fit within A4 dimensions. If content is too long, split logically across multiple pages.

Design Style: 'Academic Neumorphism' — soft elevation, minimal shadows, rounded corners, clear section demarcations, and a balanced academic look. Use subtle gradients and pastel colors for calm readability (background: #F9FAFB → #EEF2FF; accents: #6366F1, #14B8A6, #F59E0B). Apply modern fonts (Inter, Manrope, or Lexend) and consistent spacing for digestible content. CRITICAL ICON REQUIREMENT: DO NOT use emoji icons (📘, 🧠, 📚, 📋, 💡, 🔗, ⚙️, etc.) as they may not render properly in images. Instead, use HTML entities (&#9733; for star, &#9679; for bullet, &#10003; for checkmark) or CSS-styled elements (colored circles, squares with borders). Layout Flow: 1. **Header / Title Section** — Prominent title Ready Reckoner:topic of the Section with a small subtitle like 'Curated Summary | Powered by DATADNA AI Study Platform'. Include a gradient header strip and use HTML entities or CSS shapes instead of emoji icons. 2. **Concept Overview** — A concise introduction paragraph summarizing the study topic with key definitions and its importance. Use HTML entities or a highlighted box for 'Core Idea' or 'Why It Matters'. 3. **Key Concepts Grid** — A responsive grid (2–3 columns) of neumorphic cards, each representing a main concept or formula. Each card includes: Title, short explanation, and optional formula or key value. Add hover effect to reveal extra insights or quick examples. 4. **Formulae / Frameworks Section** — Use visually distinct blocks for equations, frameworks, or models (depending on the subject). For example, include Math formulas, Code snippets, or Process diagrams. Each block should have a soft background, rounded border, and use HTML entities or CSS shapes instead of emoji icons. 5. **Mind Map or Flow Summary Section** — Horizontal layout with connected visual nodes showing relationships between subtopics. Can be represented as stylized divs or HTML entities connected by gradient lines or arrows (simulate knowledge flow). 6. **Quick Reference Table** — Compact and clean table listing terms, abbreviations, or constants with 3 columns (Term | Meaning | Example). Alternate row colors for clarity. 7. **Smart Insights / AI Notes Section** — Highlighted card with HTML entity (&#128161; for lightbulb) or CSS-styled icon containing 'AI-Powered Study Tips' or 'Common Mistakes & How to Avoid Them'. Use gradient border glow and soft hover animation. 8. **Knowledge Check Section** — Optional quiz-style block with 3–5 short questions and answers (like 'Test Yourself'). Use rounded cards with subtle background contrast. 9. **Footer / Attribution** — Footer text: “© 2025 DATADNA AI Study Platform — Ready Reckoner Generated by AI”. Include small icons (🔗📚⚙️) and optional link placeholder. Typography: Use clean, readable type hierarchy — h1 bold (2xl), h2 (xl), body (base). Maintain generous padding and whitespace. Animations: Smooth fade-in for sections, hover shadow shifts for cards, and slow background gradient movement in header. Output Format: <!DOCTYPE html>...complete HTML script here.... Strictly adhere to the output format given. Additional Notes: The page must look structured, calm, and intuitive for study purposes — readable in both light and dark modes. Avoid clutter, ensure responsive alignment, and use color cues for grouping concepts. CRITICAL REQUIREMENT: The HTML must be returned as a SINGLE CONTINUOUS LINE with absolutely NO newline characters (\\n), NO line breaks, NO tabs, and NO formatting whitespace. Minify the HTML completely by removing all spaces between tags. The entire HTML must be one unbroken line from <!DOCTYPE to </html>. Do NOT format or pretty-print the HTML. Do NOT wrap the HTML in JSON. Do NOT add quotes around the HTML. Return ONLY the raw HTML code starting with <!DOCTYPE and ending with </html>, nothing else - no JSON wrapper, no markdown, no explanations.`;
}
...
function getFlashCardsPrompt(params) {
  ...
  return `Extract key concepts from section ${sectionNumber} in the chapter to form the basis of an HTML study guide and Generate an HTML script that visually presents a flash card for each key concept. Each flash card should include a question or prompt and the corresponding answer or explanation. The design should be visually appealing and easy to read, with a consistent layout and color scheme. The flash cards should be organized in a grid layout, with each card having a uniform size and shape. The overall design should be professional and suitable for use in an educational or training setting. notes of the file with ${contentDepth} content depth in ${outputLanguage} language in ${contentType} style with ${visualStyle} nature as a structured, visually elegant, and interactive reference sheet using Tailwind CSS. The layout should serve as a quick-access knowledge companion for students and professionals — focused on clarity, visual memory cues, and ease of scanning. 
...
3. Multi-Page Support: If content exceeds one A4 page, create multiple page divs. Use CSS page-break-after: always on each page div except the last one.
4. Print Styles: Include @media print rules to ensure proper page breaks: @media print { .page { page-break-after: always; } .page:last-child { page-break-after: auto; } }
5. Content Density: Optimize content to fit within A4 dimensions. If content is too long, split logically across multiple pages (e.g., 4-6 flash cards per page).

Design Style: 'Multicolor Gradient Cards' — vibrant gradient backgrounds with 10 distinct color palettes (Blue #4F46E5→#3730A3, Purple #9333EA→#7E22CE, Green #10B981→#059669, Red #FF6B6B→#EE5A52, Pink #EC4899→#DB2777, Orange #F97316→#EA580C, Teal #14B8A6→#0D9488, Cyan #06B6D4→#0891B2, Indigo #6366F1→#4F46E5, Rose #F43F5E→#E11D48), rounded corners (16px), soft shadows (0 8px 16px rgba), white text overlay, and category badges with translucent background (rgba(255,255,255,0.3)). Apply modern fonts (Inter for body, Poppins for headings) and consistent padding (2rem) for digestible content. CRITICAL ICON REQUIREMENT: DO NOT use emoji icons (📘, 💡, 🔗, 📚, ⚙️, etc.) as they may not render properly in images. Instead, use HTML entities (&#9733; for star, &#9679; for bullet, &#10003; for checkmark, &#128161; for lightbulb) or CSS-styled elements (colored circles, squares with borders). Layout Flow: 1. **Header / Title Section** — Prominent title "Flash cards: [topic]" with subtitle 'Curated Summary | Powered by DATADNA AI Study Platform' and use HTML entities or CSS shapes instead of emoji icons. Use center alignment with large, bold typography (1.25rem). 2. **Concept Overview** — A concise introduction in a neutral, clean box without gradient, summarizing the study topic with key definitions using standard text on light background (#F9FAFB). 3. **Key Concepts Grid** — A responsive 2-column grid of colorful cards (using gradient color palette), each representing a main concept. Each card includes: Category badge (top, translucent white), Title in bold (1.25rem), and short explanation (1rem). Include subtle divider line (rgba(255,255,255,0.3)) separating question from answer. 4. **Formulae / Frameworks Section** — Use one of the gradient-colored cards to display formulas/frameworks with clear typography, soft rounded borders (16px), and light text on gradient background. Include category badge and divider for visual separation. 5. **Mind Map or Flow Summary Section** — Display as a vertical layout of gradient cards connected by subtle visual flow, using the same color palette and card styling as concepts. Maintain consistent spacing and alignment. 6. **Quick Reference Table** — Compact table with neutral background (#F9FAFB), 2 columns (Term | Meaning | Example), alternate row colors for clarity, no gradients. Use Inter font with base typography size. 7. **Smart Insights / AI Notes Section** — Highlighted card using one of the vibrant gradients with HTML entity (&#128161; for lightbulb) or CSS-styled icon, containing study tips or common mistakes. Use category badge and text styling consistent with concept cards. 8. **Knowledge Check Section** — Quiz-style blocks styled as gradient cards from the color palette, each containing a question (bold, large) with answer below separated by divider line. Maintain consistent card height (min-height: 320px) and padding (2rem). 9. **Footer / Attribution** — Footer text "© 2025 DATADNA AI Study Platform — Ready Reckoner Generated by AI" on light neutral background (#F9FAFB) with center alignment and use HTML entities or CSS shapes instead of emoji icons. Typography: Use bold, large headings for h1 (2.5rem), standard bold for h2 (1.25rem), and readable body text (1rem with 1.6 line-height). Apply font-family Poppins for titles and Inter for body. Maintain generous padding (2rem) and whitespace. Animations: Smooth fade-in for sections, subtle hover shadow shifts for cards (translateY on hover), and no transition delays for immediate responsiveness. Color rotation: Distribute gradient colors across cards automatically using cycling through the 10-color palette. Output Format: <!DOCTYPE html>...complete HTML script here.... Strictly adhere to the output format given. Additional Notes: The page must look structured, calm, and intuitive for study purposes — readable in both light and dark modes. Avoid clutter, ensure responsive alignment, and use color cues for grouping concepts. CRITICAL REQUIREMENT: The HTML must be returned as a SINGLE CONTINUOUS LINE with absolutely NO newline characters (\\n), NO line breaks, NO tabs, and NO formatting whitespace. Minify the HTML completely by removing all spaces between tags. The entire HTML must be one unbroken line from <!DOCTYPE to </html>. Do NOT format or pretty-print the HTML. Do NOT wrap the HTML in JSON. Do NOT add quotes around the HTML. Return ONLY the raw HTML code starting with <!DOCTYPE and ending with </html>, nothing else - no JSON wrapper, no markdown, no explanations.`;
}
