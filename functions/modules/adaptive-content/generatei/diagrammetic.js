...
async function buildDiagramImagePromptAndDescription(diagramData, topicName) {
  const diagramType = diagramData?.diagram?.type || 'TREE';
  const nodeLabels = (diagramData?.diagram?.nodes || [])
    .map(n => n.label).slice(0, 8).join(', ');
  const coreIdea = diagramData?.coreIdea || '';
...
Generate TWO things:
1. A 2-line image generation prompt for a black & white line art diagram
2. A 3-line description explaining what the diagram shows

Return ONLY valid JSON in this exact format:
{
  "imagePrompt": "Your 2-line prompt here...",
  "imageDescription": "Line 1 describing the diagram. Line 2 continuing the explanation. Line 3 finishing the description."
}

STRICT RULES for imagePrompt:
- Black and white line art only
- Pure white background
- Thin black ink strokes
- NO text, NO labels, NO letters, NO numbers anywhere
- Educational textbook illustration style

Keep imagePrompt concise (2 lines maximum).`;
  ...
  // Fallback
  return {
    imagePrompt: `Black and white line art diagram of ${subject}. Key concepts: ${nodeLabels}. Educational textbook style, no text, no labels.`,
    imageDescription: `This diagram illustrates the key concepts of ${subject}. The visual representation shows how different components relate to each other. Understanding this structure helps grasp the core educational concepts effectively.`
  };
}

async function generateAndUploadDiagramImage(diagramData, topicName, existingPrompt = null) {
  try {
    let imagePrompt;
    ...
    if (existingPrompt) {
      ...
      imageDescription = diagramData?.imageDescription || '';
    } else {
      // Generate both prompt and description
      const result = await buildDiagramImagePromptAndDescription(diagramData, topicName);
    ...
    return { 
      ...
      imageId, 
      imagePrompt, 
      imageDescription: imageDescription || diagramData?.imageDescription || '' 
    };
